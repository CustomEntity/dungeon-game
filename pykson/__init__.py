...
class JsonObjectMeta(type):
    ...
    def __new__(mcs, name, bases, attrs: Dict[str, Any]):
        ...
        def my_custom_init(instance_self, *init_args, **init_kwargs):
...
class Pykson:
    ...
    def _from_json_dict(self, data: Dict, cls: Type[T]) -> T:
        sub_type = cls
        ...
        for data_key, data_value in data.items():
            ...
            #             data_copy[data_key] = data_value
                ...
                if data_key in field_names_mapped_by_serialized_names.keys():
                    ...
                    data_copy[data_key] = data_value
        return sub_type(**data_copy)

    # noinspection PyCallingNonCallable
    def _from_json_list(self, data: List, cls: Type[T]) -> List[T]:
        list_result = []  # type: List[T]
        for data_value_item in data:
            # noinspection PyUnresolvedReferences
            list_result.append(self.from_json(data_value_item, cls))
        return list_result

    def from_csv(self, data: str, cls: Type[T], line_separator: str = '', first_row_as_field_names: bool = True) -> List[T]:
        ...
        return self._from_json_list(rows, cls=cls)

    def from_json(self, data: Union[str, Dict, List], cls: Type[T]) -> Optional[Union[T, List[T]]]:
        if isinstance(data, str):
            data = json.loads(data)
        if isinstance(data, dict):
            return self._from_json_dict(data, cls)
        elif isinstance(data, list):
            return self._from_json_list(data, cls)
    ...
    def to_json(self, item: Union[T, List[T]]) -> str:
        return json.dumps(self._to_json(item), indent=2)

    def to_json_file(self, item: Union[T, List[T]], file):
        return json.dump(self._to_json(item), file, indent=2)
